...
let baseURI = '/apps/' + config.HEROKU.APP_NAME;
   ...
   var l_dsc = ''
    ...
    inrl.addCommand({pattern: 'bgm ?(.*)', fromMe: true, desc: l_dsc, usage: '.bgm on / off' }, (async (message, match) => {
        if (match[1] == 'off') {
    ...
    inrl.addCommand({pattern: 'autosticker ?(.*)', fromMe: true, desc: Y_dsc, usage: '.sticker on / off' }, (async (message, match) => {
        if (match[1] == 'off') {
                await heroku.patch(baseURI + '/config-vars', { 
                    body: { 
                        ['AUTO_STICKER']: 'false'
    ...
    inrl.addCommand({ pattern: 'sudo ?(.)', fromMe: true, desc: 'changes sudo numbers', usage: '.sudo *your number' }, (async (message, match) => {
    ...
    inrl.addCommand({ pattern: 'caption ?(.)', fromMe: true, desc: 'changes all captions', usage: '.caption *ᴍᴀᴅᴇ ʙʏ ᴘʀɪɴᴄᴇ ʀᴜᴅʜ' }, (async (message, match) => {
    ...
    inrl.addCommand({ pattern: 'handlers ?(.*)', fromMe: true, desc: 'changes handlers', usage: '.handler ^[.!] ' }, (async (message, match) => {
    ...
    inrl.addCommand({ pattern: 'botname ?(.)', fromMe: true, desc: 'change your bot name', usage: '.botname *name ' }, (async (message, match) => {
...
inrl.addCommand({ pattern: 'theri  ?(.*)', fromMe: true, desc: 'change your theri commands', usage: '.theri command,command' }, (async (message, match) => {
        if (match[1] == '') return await message.sendMessage('TYPE YOUR NEW BOT NAME')
        await heroku.patch(baseURI + '/config-vars', {
            body: {
                ['THERI_LIST']: match[1]
